...
- 导出候选股票 K 线图
- 调用 Qwen3.5-plus 对图表进行 AI 视觉复评打分
...
- AI 复评从 Gemini 迁移至通义千问 Qwen3.5-plus
...
~~~powershell
[Environment]::SetEnvironmentVariable("DASHSCOPE_API_KEY", "你的DashScopeApiKey", "User")
~~~
...
写入后请重开终端，环境变量才会在新会话中生效。DashScope API Key 在阿里云百炼控制台获取。
...
### Q1：fetch_kline 报网络错误或被限流

- AKShare 无需 token，但有并发频率限制
- 降低 `workers` 数量（建议 3～5）
- 遇到限流会自动等待重试，属于正常现象
...
- 确认模型名称 `qwen3.5-plus` 支持视觉输入，否则换用 `qwen-vl-plus`
